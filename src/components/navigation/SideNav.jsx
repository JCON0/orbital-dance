...
const SideNav = ({ isOpen = true }) => {
  ...
  return (
    <aside
      className={`fixed left-0 top-0 hidden h-screen w-64 shrink-0 bg-slate-950 p-5 shadow-sm border-slate-700 overflow-y-auto lg:block transition-transform duration-300 ease-in-out ${
    ...
    >
      <div className="flex h-full flex-col">
        <div className="flex items-center gap-2 px-1 pb-6 pt-2">
          ...
          <div className="text-xl font-bold text-white">Orbital Dance</div>
        </div>

        <nav className="flex flex-col gap-2">
          <h1 className="text-sm font-bold text-slate-300 px-4 py-3">
            Main menu
          </h1>
          {navItems.map((item) => (
        ...
        <div className="rounded-2xl bg-cyan-900/20 px-4 py-3">
          <p className="text-xs font-semibold uppercase tracking-wider text-cyan-300">Need help?</p>
          <p className="mt-1 text-xs text-cyan-400">Check our documentation or contact support for assistance.</p>
        </div>
